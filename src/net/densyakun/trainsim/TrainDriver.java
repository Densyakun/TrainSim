...
public final class TrainDriver implements Serializable {
	...
	public void updatetick(RailwayManager railwaymanager, Train train) {
		...
		if (diagram != null) {// ダイヤがあるか
			...
			if (nextstop != null) {// 次の停車駅があるか
					...
					for (int g = 0; g < runroute.size() && e <= trainlengthhalf; g++) {
						...
						if (f) {
							...
							for (int i = 0; i < rails.size() && e <= trainlengthhalf; i++) {
								...
								if (h ? position - trainlengthhalf <= line.getRailStartPosition(rail) + rail.getLength()
										: true) {
				...
				double distanceto_ = distanceto;
				if (speed != 0 && !right) {
					distanceto_ = -distanceto_;
				}
				brake = Math.max(distanceto_ <= stoppable_distance ? 1 : stoppable_distance / distanceto_, brake);
				...
				if (brake != 1) {
					...
					for (int c = 0; c < trains.size(); c++) {
						...
						if (d != train && train.getRunningLine().equals(d.getRunningLine())
								&& position < d.getPosition() && position + 200 + trainlengthhalf >= d.getPosition()
										- d.getTrainset().getLength() / 2) {
							brake = 1;
							break;
						}
					}
					...
					double targetspeed = acceled ? maxspeed - 5 : maxspeed;// 運転士が出したい目標の速度（再加速を安定させるため、acceledがtrueの場合は目標最高速度
																			// -
																			// 5キロになってから再加速するようにしている）
	...
	/*private static Double aaa(double distance, double limitdistance, int limitspeed, List<Line> lines, Line line,
			boolean right) {
		...
		if (right) {
			...
			double a = line.getRailStartPosition();
			if (a != -1) {
				return distance + a;
			}
			if (0 <= line.getBranch_out()) {
				...
				lines.add(line_out);
				return aaa(distance, limitdistance, limitspeed, lines, line_out, true);
			}
		} else {
			double a = line.getRailStartPosition();
			if (a != -1) {
			...
			if (0 <= line.getBranch_in()) {
				...
				lines.add(0, line_in);
				return aaa(distance, limitdistance, limitspeed, lines, line_in, false);
			}
		}
		return null;
	}*/
